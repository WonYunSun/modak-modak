...
const CommentCard = ({ comment, postId }: CommentCardProps) => {
  ...
  const [user, setUser] = useState<any>(null); // 타입은 실제 User 타입

  const { setActionModalOpen } = useActionModal();

  const { setCheckModify, setCommentValue, setCommentId, checkModify } = useCommentValue();
...
//   const { setActionModalOpen } = useActionModal();
//   const { setCheckModify, setCommentValue, setCommentId } = useCommentValue();
//   const { deleteCommentMutation, updateCommentMutation } = useCommentHandler(comment.id, postId);
...
//         </div>
//         <Menu className="w-6 h-6 cursor-pointer" onClick={handleSheetOpen} />
//         {/* {user.id === comment.user_id && <Menu className="w-6 h-6 cursor-pointer" onClick={handleSheetOpen} />} */}
//         <BottomSheet isOpen={openSheet} onClose={handleSheetClose} snapPoint={[0.2]}>
//           <div className="w-full h-full bg-white rounded-xl divide-y flex flex-col justify-start overflow-hidden">
//             <Button
//               type="button"
